...
AnimatedSprite::AnimatedSprite(const std::vector<Sprite> &sprites,
                               double dt_per_frame, int starting_frame)
    : sprites{sprites},
...
Sprite AnimatedSprite::get_sprite() const { return sprites.at(current_frame); }

int AnimatedSprite::number_of_frames() const { return sprites.size(); }

void AnimatedSprite::shift(Vec<int> shift)
{
    for (auto &sprite : sprites)
    {
        sprite.shift += shift;
