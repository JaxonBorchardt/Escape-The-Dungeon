...
World::World(const Level &level) : tilemap{level.width, level.height}, backgrounds{level.backgrounds}, quadtree{AABB{{level.width / 2.0, level.height / 2.0}, {level.width / 2.0, level.height / 2.0}}}
{
...
std::shared_ptr<Command> World::touch_tiles(const Player &player)
{
    ...
    const std::vector<Vec<int>> displacemnets{{0, 0}, {size.x, 0}, {0, size.y}, {size.x, size.y}};

    for (const Vec<int> &displacemnet : displacemnets)
    {
        Tile &tile = tilemap(x + displacemnet.x, y + displacemnet.y);
        if (tile.command)
        {
