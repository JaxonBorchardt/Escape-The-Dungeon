...
void QuadTree::subdivide()
{
    ...
    north_west = std::make_shared<QuadTree>(AABB{
        {boundary.center.x - half.x, boundary.center.y + half.y}, half});

    north_east = std::make_shared<QuadTree>(AABB{
        {boundary.center.x + half.x, boundary.center.y + half.y}, half});

    south_west = std::make_shared<QuadTree>(AABB{
        {boundary.center.x - half.x, boundary.center.y - half.y}, half});

    south_east = std::make_shared<QuadTree>(AABB{
        {boundary.center.x + half.x, boundary.center.y - half.y}, half});
